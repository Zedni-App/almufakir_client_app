...
class HomeBloc extends Bloc<HomeEvent, HomeState> {
  ...
  FutureOr<void> _getCourses(GetCourses event, Emitter<HomeState> emit) async {
    emit(LoadingCoursesState());
    final reqResult = await sl<GetComboUseCase>().call();
    reqResult.fold(
      (l) => emit(LoadingCoursesFailedState(errorMessage: l.message)),
      (r) => emit(LoadingCoursesDoneState(data: r)),
    );
  }
